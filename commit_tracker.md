...
- Executed the plan section 10.7 support-promise audit against the current matrix: `docs/support/support-policy.md`'s environment bullets now mirror the matrix quality labels exactly, adding an explicit payload-inspected bullet for deb/rpm/Pacman so built-but-never-executed formats carry no executed-behavior promise; the maintenance document, known-limitation cross-references, and release documents passed the over-promise sweep with no other drift. The checkbox stays open for a final sweep after freeze completes the matrix.
