...
**Status:** Implemented Phase 3 scope; Phase 4 keyboard, layout, and axe-core evidence recorded; semantic-structure and announcement contracts verified August 22, 2026; manual screen-reader campaign in progress
...
- Assistive-technology observations (Phase 4): `tests/accessibility/assistive-technology.spec.ts` records four automated findings on the built application: (1) with platform accessibility enabled, the real Chromium accessibility tree exposes every route's heading, named controls, the chart image alternative, and settings radios; (2) under emulated `prefers-reduced-motion: reduce` the shipped spinner computes as non-animated while animating at baseline; (3) under emulated forced colors the UA palette replaces authored backgrounds while core controls keep geometry, text, and foreground; (4) countdown metrics sit outside all live-region ancestry, so thirty-second clock ticks cannot produce repeated announcements, and live status remains reserved for refresh-state changes.
- Semantic-structure and announcement contracts (Phase 6 preparation, August 22, 2026): `tests/e2e/semantic-structure.spec.ts` verifies on the built application that the landmark inventory is stable (exactly one main landmark, named complementary and navigation landmarks, first-focus skip link), every route carries exactly one level-one heading inside the main landmark, every interactive control on Overview, Settings & Diagnostics, and Learn carries a non-empty accessible name, signed-out guidance renders as a named region rather than an alert, no assertive alert exists in resting states, and `routes.test.tsx` pins the live-region semantics at component level: a stale snapshot announces through `role="alert"` with its retry action inside the same announcement, the repeating connection indicator stays a polite `role="status"`, and fresh states expose no alerts.

## Known limitations

- The human Orca screen-reader session on representative Linux desktops remains outstanding manual evidence; it requires operator judgment that automation cannot substitute for, and will be recorded when performed.
- Live-region announcements for background snapshot pushes are not yet distinguished from user-triggered refreshes (automatic refresh ships off by default, so background pushes do not occur in v1 as configured).
