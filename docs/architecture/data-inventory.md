...
| Account summary | Codex app-server | Not accessed | Connection and plan context | Memory only | Normalized minimal fields | Identifiers excluded |
| Quota snapshots | Codex app-server | Not accessed | Quota windows and derived explanations | Memory only | Validated domain DTO | Values excluded by default |
