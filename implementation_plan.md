...
**Current phase:** v1.0.0 published on August 22, 2026: the manifest was frozen at `1.0.0` in one reviewed commit (`5d4e307`), the changelog entry finalized as `## 1.0.0`, the companion website taken live with released-state copy, and the full local gate matrix re-executed green on that exact freeze/tag commit (format, lint, five-project typecheck, 229 unit tests, 32 integration tests, budget-gated production build, 32 e2e, 8 accessibility/development, 3 security, 4 packaged, performance gate, 55-file documentation check). The tag-driven pipeline built one reviewed draft (run `32601895640`, green), the pre-publication repository settings were completed — required reviewer on the `release` environment and active ruleset `21213599` making `v*` tags immutable, clearing former LIM-002 — and the draft was published at https://github.com/amanalip/tokentrail/releases/tag/v1.0.0 with all eight public artifacts checksum-verified after download and a public-location AppImage launch observed on the reference machine. Remaining open items stay operator-held or environment-bound: executed install/upgrade/uninstall campaigns per format, second-family AppImage coverage, arm64 execution, external security review, guide follow-through, soak and network-trace capture, and the versioned `1.0.0` report
...
- [x] Add release-note structure for highlights, security, fixes, known limitations, installation links, checksums, and upgrade notes.
  - Draft notes are now generated by `scripts/write-release-notes.mjs` from the tagged commit's changelog (version section, or the honestly labeled `Unreleased` fallback) plus fixed operational sections; verified locally against the real changelog with fail-closed behavior on malformed inputs. The workflow's draft job checks out the tag, re-verifies tag/manifest agreement, and runs the generator before `gh release create`.
