...
**Current phase:** Phase 6 candidate freeze executed on August 22, 2026: the manifest was frozen at `1.0.0` in one reviewed commit, the changelog entry was finalized as `## 1.0.0`, and the companion website was updated to describe the released state before tagging. Local gates re-verified green on the freeze commit (format, lint, five-project typecheck, 229 unit tests, 32 integration tests, budget-gated production build). The full automated matrix (e2e, accessibility/development, security, packaged, performance) last ran green against commit `1308e1a`; rerunning it against the exact release tag commit remains a checklist item where this environment allows. Remaining open items stay operator-held or environment-bound: protected-environment reviewers and tag immutability settings, executed install/upgrade/uninstall campaigns per format, second-family AppImage coverage, arm64 execution, external security review, guide follow-through, soak, and publication approval
