...
**Current phase:** Phase 5 closed at the machine-verifiable limit; Phase 6 preparation is underway on August 22, 2026. Since the Phase 5 close-out: the root changelog and the release/support/maintenance documentation set landed, the companion website shipped to GitHub Pages, and draft releases now carry generated structured notes (highlights, security, fixes, checksums, install links, upgrade notes, known limitations) built from the tagged changelog — closing the last machine-verifiable section 9.4 item. Remaining open items are operator-held or environment-bound: protected-environment reviewers and tag immutability settings, executed install/upgrade/uninstall campaigns per format, second-family AppImage coverage, arm64 execution, external security review, guide follow-through, the v1.0.0 freeze, full validation matrix, soak, and publication approval
